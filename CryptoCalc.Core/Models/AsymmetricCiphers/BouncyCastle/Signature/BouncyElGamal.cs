...
{
    public class BouncyElGamal : IAsymmetricEncryption, INonECAlgorithims
    {
        ...
        /// </summary>
        private IAsymmetricBlockCipher cipher = new ElGamalEngine();
        ...
        public ObservableCollection<int> GetKeySizes()
        {
            ...
            for(int strength = 1024; strength <= maxStrength; strength += 256)
            {
        ...
        {
            var p = new BigInteger(keySize, new Random());
            var g = new BigInteger(keySize, new Random());
            var elGamalParameters = new ElGamalParameters(p, g);
            var parameters = new ElGamalKeyGenerationParameters(new SecureRandom(), elGamalParameters);
        ...
        {
            var x = ((ElGamalPrivateKeyParameters)keyPair.Private).X.ToByteArrayUnsigned();
            var p = ((ElGamalPrivateKeyParameters)keyPair.Private).Parameters.P.ToByteArrayUnsigned();
            var g = ((ElGamalPrivateKeyParameters)keyPair.Private).Parameters.G.ToByteArrayUnsigned();
            var l = ((ElGamalPrivateKeyParameters)keyPair.Private).Parameters.L;
        ...
        {
            var y = ((ElGamalPublicKeyParameters)keyPair.Public).Y.ToByteArrayUnsigned();
            var p = ((ElGamalPrivateKeyParameters)keyPair.Private).Parameters.P.ToByteArrayUnsigned();
            var g = ((ElGamalPrivateKeyParameters)keyPair.Private).Parameters.G.ToByteArrayUnsigned();
            var l = ((ElGamalPrivateKeyParameters)keyPair.Private).Parameters.L; 
        ...
        /// <summary>
        /// Creates a public key <see cref="ElGamalPublicKeyParameters"/> from a byte array containing the exponent and modulus
        /// </summary>
        ...
        private ElGamalPublicKeyParameters CreatePublicKeyParameterFromBytes(byte[] publicKey)
        {
            ...
            return new ElGamalPublicKeyParameters(exponent, new ElGamalParameters(new BigInteger(1, new Random()), new BigInteger(1, new Random()), 2));
        }

        /// <summary>
        /// Creates a private key <see cref="ElGamalPrivateKeyParameters"/> from a byte array containing the exponent and modulus
        /// </summary>
        /// <param name="publicKey">the byte array conatining the exponent and the modulus</param>
        /// <returns>The private RSA key parameter object</returns>
        private ElGamalPrivateKeyParameters CreatePrivateKeyParameterFromBytes(byte[] privateKey)
        {
            ...
            var modulus = new BigInteger(1, m);
            var exponent = new BigInteger(1, e);
            return new ElGamalPrivateKeyParameters(exponent, new ElGamalParameters(new BigInteger(1, new Random()), new BigInteger(1, new Random()), 2));
        }
